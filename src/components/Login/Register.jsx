...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    async function register(){
      ...
      let result = await fetch('http://127.0.0.1:8000/authenticate/user/signup', options)
      .then((response) => { return response.text()
        .then((data) => {  return data; })
        .then((data) => localStorage.setItem('register', data))
        ...
        if(localStorage.getItem('register') === 'SignedUP'){
          navigate('/');
        }
    }
      ...
      <div className="lg:flex">
        <div className="lg:w-1/2 xl:max-w-screen-sm">
          <div className="mt-10 px-12 sm:px-24 md:px-48 lg:px-12 lg:mt-16 xl:px-24 xl:max-w-2xl">
            <h4
              className="text-center text-xl text-indigo-900 font-display font-semibold lg:text-left xl:text-4xl
                    xl:text-bold"
            >
              Register
            </h4>
